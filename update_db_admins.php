...
if(mysqli_num_rows($check_admin_bdg) == 0) {
    ...
    mysqli_query($koneksi, "UPDATE admin_users SET password = '$pass_bdg', role = 'admin', id_cabang = 2 WHERE username = 'admin_bandung'");
    echo "<div>✓ Admin Bandung account password reset/checked.</div>";
}
...
if(mysqli_num_rows($check_admin_sby) == 0) {
    ...
    mysqli_query($koneksi, "UPDATE admin_users SET password = '$pass_sby', role = 'admin', id_cabang = 3 WHERE username = 'admin_surabaya'");
    echo "<div>✓ Admin Surabaya account password reset/checked.</div>";
}
